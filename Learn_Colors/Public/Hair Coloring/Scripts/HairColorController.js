// HairColorController.js
// Version: 0.0.2
// Event: Initialized
// Description: Sets various coloring modes for the hair effect

//@input vec3 colorVal1 = {0.0, 0.0, 0.0} {"widget":"color", "label":"Base color"}
//@input int mode = 0 {"widget":"combobox", "values":[{"label":"Full head", "value":0}, {"label":"Ends", "value":1}, {"label":"Roots", "value":2}]}

//@input bool secondColor = false
//@ui {"widget":"group_start", "label":"Additional color", "showIf":"secondColor"}

//@input vec3 colorVal2 = {0.2, 0.5, 0.96} {"label":"Secondary color", "widget":"color"}
//@input int gradientMode = 0 {"widget":"combobox", "values":[ {"label":"Ombre", "value":0}, {"label":"Split", "value":1}], "showIf":"mode", "showIfValue":0} 
//@input bool swapColors = false

//@ui {"widget":"group_end"}

//@input bool advanced = false
//@ui {"widget":"group_start", "showIf":"advanced"}
//@input Component.Image hairColorImage {"showIf":"advanced"}
//@input Component.PostEffectVisual hairColorEffect {"showIf":"advanced"}
//@input Component.Camera camera {"showIf":"advanced"}
//@ui {"widget":"group_end"}

var sceneObject = global.scene.createSceneObject("HeadObject");
var head = sceneObject.createComponent("Head");
head.faceIndex = 0;
head.setAttachmentPointType(AttachmentPointType.CandideCenter);
var pass = undefined;
var gradientMode = script.gradientMode;

var red = new vec3(1.00, 0.00, 0.00);
var yellow = new vec3(1.00, 0.96, 0.00);
var purple = new vec3(0.55, 0.00, 0.52);
var green = new vec3(0.34, 0.83, 0.07);
var blue = new vec3(0.00, 0.00, 0.95);
var orange = new vec3(0.98, 0.42, 0.00);

if (script.hairColorImage == null) {
    print("HairColorController, Warning: Please Hair Color Image is not set");
}

if (!script.hairColorEffect) {
    print("HairColorController, Error: Please set hairColorEffect object in Advanced section");
    return;
}

if (!script.camera) {
    print("HairColorController, Error: Please set camera object in Advanced section");
    return;
}

if (script.mode == 0 && !script.secondColor) {
    //single color full 0
    gradientMode = 0;
}
if (script.mode == 1 && !script.secondColor) {
    //single color ends 2
    gradientMode = 2;
}
if (script.mode == 2 && !script.secondColor) {
    //singles color roots 3
    gradientMode = 3;
}
if (script.mode == 0 && script.secondColor) {
    if (script.gradientMode == 0) {
        //double color full 1
        gradientMode = 1;
    }
    if (script.gradientMode == 1) {
        //double split full 4
        gradientMode = 4;
    }
}
if (script.mode == 1 && script.secondColor) {
    //double split ends 5
    gradientMode = 5;
}
if (script.mode == 2 && script.secondColor) {
    //double split roots 6
    gradientMode = 6;
}
    
var colorVal1 = script.colorVal1,
    colorVal2 = script.colorVal1;
   
if (script.colorVal1) {
    colorVal1 = script.colorVal1;
       // change hair color
    print("Hair color current is: " + global.tweenManager.getLastKnownColor());
    switch(global.tweenManager.getLastKnownColor()) {
      case "red":
        colorVal1 = red;
        break;
      case "yellow":
        colorVal1 = yellow;
        break;
      case "purple":
        colorVal1 = purple;
        break;
      case "green":
        colorVal1 = green;
        break;
      case "blue":
        colorVal1 = blue;
        break; 
      case "orange":
        colorVal1 = orange;
        break;  
    }    
}

if (script.secondColor) {
    colorVal2 = script.colorVal2;
}
    
if (script.swapColors) {
    var tmp = colorVal1;
    colorVal1 = colorVal2;
    colorVal2 = tmp;
}

var updateEvent = script.createEvent("UpdateEvent");
updateEvent.bind(onUpdate);

function onUpdate(eventData) {
    if (head.getFacesCount() > 0) {
        var transform = head.getTransform();
        var center1 = transform.getWorldPosition();        
        var center2 = center1;
        center2.y += 20.0;

        var projectedCenter1 = script.camera.project(center1);
        var projectedCenter2 = script.camera.project(center2);
        projectedCenter2 = projectedCenter2.sub(projectedCenter1);
        var size = 1.7 * projectedCenter2.y * projectedCenter2.y;

        var eulerAngles = transform.getLocalRotation().toEulerAngles();
        pass.facea = new vec4(0.5 + projectedCenter1.x, 0.5 + projectedCenter1.y, size, Math.sin(eulerAngles.x));
        var vectorFace = new vec2(transform.left.x, transform.left.y);
        pass.facedir = vectorFace.normalize();
    }
}

function enable() {
    if (script.hairColorEffect.mainPass) {
        pass = script.hairColorEffect.mainPass;
        
        pass.useGradient = gradientMode + 0.0;
        pass.facea = new vec4(0.5,0.5,0.5,0);
        pass.facedir = new vec2(-1.0, 0.0);
        
        pass.baseColor1 = colorVal1;
        if (script.secondColor) {
            pass.baseColor2 = colorVal2;
        }
        
        script.hairColorEffect.enabled = true;
        
        if (script.camera == null) {
            print("HairColorController, Error: Please set Camera object in Advanced section");
            return;
        }
    } else {
        print("HairColorController, Error: Please set Hair Color object in Advanced section");
        return;
    }
    
    if (script.hairColorImage) {
        script.hairColorImage.enabled = true;
    } 
    updateEvent.enabled = true;
}

function disable() {
    if (script.hairColorImage) {
        script.hairColorImage.enabled = false;
    }
    updateEvent.enabled = false;
}



enable();

script.api.enable = enable;
script.api.disable = disable;

